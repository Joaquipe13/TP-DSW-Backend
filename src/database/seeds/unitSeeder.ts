...
export const UNITS_DATA: UnitData[] = [
  ...
  },
  {
    ...
    levelName: "Introduccion a Node.js",
    name: "Que es Node.js",
    order: 1,
...
export class UnitSeeder extends Seeder {
  async run(em: EntityManager): Promise<void> {
    try {
      console.log('Starting UnitSeeder...');

      for (const unitData of UNITS_DATA) {
        const course = await em.findOne(Course, { title: unitData.courseTitle });
        if (!course) {
          console.warn(`Course not found: ${unitData.courseTitle}`);
          continue;
        }

        const level = await em.findOne(Level, {
          name: unitData.levelName,
          course: course,
        });

        if (!level) {
          console.warn(`Level not found: ${unitData.levelName} (${unitData.courseTitle})`);
          continue;
        }

        const existingUnit = await em.findOne(Unit, {
          name: unitData.name,
          level: level,
        });

        if (existingUnit) {
          console.log(`Unit already exists: ${unitData.name} (${unitData.levelName})`);
          continue;
        }
      ...
      await em.flush();
      console.log('UnitSeeder completed');
    } catch (error: any) {
      throw new Error(error.message ? `Error in UnitSeeder: ${error.message}` : 'Error in UnitSeeder: Unknown error');
    }
  }
}
